...
def sidebar_main(country: pd.Series, group: pd.Series, methodology: pd.Series) -> tuple:
    ...
    with st.sidebar:
        ...
        selected_countries = st.sidebar.multiselect(
            "Countries", country_options, placeholder="All countries", key="country_filter", on_change=keep_state, args=["country_filter"]
        )

        load_state("group_filter", [])
        selected_group = st.sidebar.multiselect(
            "Reporting group", group_options, max_selections=1, placeholder="All groups", key="group_filter", on_change=keep_state, args=["group_filter"]
        )

        load_state("method_filter", [])
        selected_methods = st.sidebar.multiselect(
            "Methodologies", method_options, placeholder="All methodologies", key="method_filter", on_change=keep_state, args=["method_filter"]
        )
...
def draw_completes_barchart(agg_df, max_daily_completes, back_days):
    ...
    for i, row in agg_df.iterrows():
        ...
        bar_valid = ax.bar(row["Date"], row["Valid Completes"], color=color_comp if i != max_idx_valid else max_bar_color_comp, alpha=alpha_comp)
        bar_invalid = ax.bar(row["Date"], row["Invalid Completes"], color=color_invalid if i != max_idx_invalid else max_bar_color_invalid, alpha=alpha_invalid)

        # Add value labels to all bars
        if row["Date"] != max_date_valid:
            ax.text(row["Date"], row["Valid Completes"] + (max_daily_completes * 0.02), f"{row['Valid Completes']}", 
                ha="center", fontsize=6, color="black", rotation=0)
        if row["Date"] != max_date_invalid:
            ax.text(row["Date"], row["Invalid Completes"] + (max_daily_completes * 0.02), f"{row['Invalid Completes']}", 
                ha="center", fontsize=6, color="white", rotation=0)
    ...
    ax.text(max_date_valid, max_value_valid + (max_daily_completes * 0.05), f"{max_value_valid}", color="black", 
        ha="center", fontsize=axis_label_size, fontweight="bold")
    ax.text(max_date_invalid, max_value_invalid + (max_daily_completes * 0.05), f"{max_value_invalid}", color="white", 
        ha="center", fontsize=axis_label_size, fontweight="bold")

    # Add median lines
    ax.axhline(median_valid, color=max_bar_color_comp, linestyle="--", linewidth=1, label=f"Median Valid Completes ({int(median_valid)})")
    ax.axhline(median_invalid, color=max_bar_color_invalid, linestyle="--", linewidth=1, label=f"Median Invalid Completes ({int(median_invalid)})")
    ...
    ax.set_xticklabels(pd.Series(xticks).dt.strftime("%b %d"), rotation=45, fontsize=tick_label_size)
    ...
    ax.legend(
        ...
        handler_map={
            ...
            f"Median Valid Completes ({int(median_valid)})": plt.Line2D([], [], color=max_bar_color_comp, linestyle="--", alpha=1),
            f"Median Invalid Completes ({int(median_invalid)})": plt.Line2D([], [], color=max_bar_color_invalid, linestyle="--", alpha=1),
        },
    )
